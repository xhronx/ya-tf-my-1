...
resource "null_resource" "docker-swarm-manager-token" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep 10",
      "echo -------------------------------------------------------SOCK_DEPLOY_COMPLETED-------------------------------------------------------",
    ]
  }

  provisioner "local-exec" {
    command = "TOKEN=$(ssh -i ${var.ssh_credentials.private_key} -o StrictHostKeyChecking=no ${var.ssh_credentials.user}@${yandex_compute_instance.vm-manager[0].network_interface.0.nat_ip_address} docker swarm join-token -q worker); echo \"#!/usr/bin/env bash\nsudo docker swarm join --token $TOKEN ${yandex_compute_instance.vm-manager[0].network_interface.0.nat_ip_address}:2377\nexit 0\" >| join_w.sh"
  }
  provisioner "local-exec" {
    command = "TOKEN=$(ssh -i ${var.ssh_credentials.private_key} -o StrictHostKeyChecking=no ${var.ssh_credentials.user}@${yandex_compute_instance.vm-manager[0].network_interface.0.nat_ip_address} docker swarm join-token -q manager); echo \"#!/usr/bin/env bash\nsudo docker swarm join --token $TOKEN ${yandex_compute_instance.vm-manager[0].network_interface.0.nat_ip_address}:2377\nexit 0\" >| join_m.sh"
  }
